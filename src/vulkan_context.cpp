...
namespace ExtImGui
{
	namespace Vulkan
	{
		...
		void Init()
		{
			...
			VkResult err = glfwCreateWindowSurface(g_Instance, GLFW::GetWindow(), g_Allocator, &surface);
			check_vk_result(err);

			// Create Framebuffers
			int w, h;
			glfwGetFramebufferSize(GLFW::GetWindow(), &w, &h);
			glfwSetFramebufferSizeCallback(GLFW::GetWindow(), GLFW::GetResizeCallback());
